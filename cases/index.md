---
page_type: cases_hub
language: zh-CN
status: draft
updated_at: 2026-04-09
---

# Case Studies

<div class="product-hero product-hero-soft product-hero-compact">
  <div class="product-hero-eyebrow">Examples</div>
  <h2>从学校页进入案例内容，再回看背景与来源。</h2>
  <p>Case Studies 区域用于承接公开案例入口，帮助用户在学校背景、来源依据与案例样本之间来回对照，而不是孤立地看单个案例。</p>
</div>

## 当前可进入的页面
### 按学校进入
- [Harvard Case Study](../schools/harvard/case-study.md)
- [Stanford Case Study](../schools/stanford/case-study.md)
- [Princeton Case Study](../schools/princeton/case-study.md)
- [Yale Case Study](../schools/yale/case-study.md)
- [Columbia Case Study](../schools/columbia/case-study.md)
- [Cornell Case Study](../schools/cornell/case-study.md)
- [UC Berkeley Case Study](../schools/uc-berkeley/case-study.md)
- [UChicago Case Study](../schools/uchicago/case-study.md)

### 按筛选进入
- [Case Study Explorer](../explorers/case-study-explorer.md)

## 现在适合怎么用
- 如果你已经知道学校：从学校页或学校 Case Study 页进入
- 如果你想按条件筛选：直接进入 [Case Study Explorer](../explorers/case-study-explorer.md)
- 如果你想先看最完整的案例路径：先看 [Harvard](../schools/harvard/index.md)

## 当前建设状态
当前阶段以清晰入口与主路径体验为主；Harvard 已可作为完整样本入口，Stanford 保持标准页面结构。 

## 关联入口
- [Universities](../indexes/universities-index.md)
- [Explorers](../explorers/index.md)